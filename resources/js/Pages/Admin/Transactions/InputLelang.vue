...
const lelangData = async () => {
    ...
    await axios.get(route("getLelangApprove")).then((res) => {
        const data = res.data;
        lelang.value = data;
...
const searchLelang = computed(() => {
    ...
    if (lelang.value) {
        return lelang.value.filter((lg) => {
            return lg.NOMOR_LELANG.toLowerCase().includes(lowerCase);
        });
    } else {
        return [];
    }
...
const selectDataLel = async (d) => {
    try {
        ...
        const response = await axios.post(route("getLelangByNomorApprove"), {
...
const generatePDF = async () => {
    try {
        ...
        const response = await axios.get(route("generatepdfKwitansi"), {
            responseType: "blob",
            params: {
                nomor_lelang: field.no_lelang,
            },
        });
        ...
        Swal.fire({
            title: "Selesai!",
            text: "Kwitansi anda siap dicetak.",
            icon: "success",
            iconColor: "#FFFFFF",
            background: "#0369a1",
            showConfirmButton: false,
...
const no_kwitansi = ref("");
const generateNoKwitansi = async () => {
    await axios.get(route("getNoKwitansi")).then((res) => {
        const data = res.data;
        no_kwitansi.value = data;
        console.log(no_kwitansi.value);
    });
};
        ...
        try {
            ...
            const lelangArray = {
                lelangTemp: lelangTemp.value.flat().map((item) => ({
                    ...
                    NO_KWITANSI: no_kwitansi.value,
                    ...item,
                })),
            };
            // Perform the API request
            await axios.post(route("storeLelangApprove"), lelangArray, header);

            Swal.fire({
                title: "Berhasil",
                text: "Menginput data lelang",
                icon: "success",
                iconColor: "#FFFFFF",
                background: "#0369a1",
                showConfirmButton: false,
...
<template>
    <Head title="Input Lelang" />
    <AdminLayout>
        <template #titlePage>
            <TitlePage value="Input Lelang Yang Telah Disetujui" />
        </template>
        <template #content>
            <main class="p-6 bg-gray-100 flex-grow overflow-y-auto">
                <div class="mt-6">
                    <div class="bg-white p-4 rounded-lg shadow">
                        ...
                        >
                            <template #title> Daftar Lelang Approved</template>
                            <template #content>
                                <div class="mt-6">
                                    <div class="pr-2-">
                                        <SearchInput
